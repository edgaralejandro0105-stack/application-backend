...
async function runTest() {
  ...
  try {
    ...
    await authService.recoverPassword(testEmail);
    ...
    console.log('Token guardado en BD:', testUser.reset_token);
    console.log('Fecha expiración:', testUser.reset_token_expires);

    console.log('\n--- 4. Simulando Reset con el Token ---');
    await authService.resetPassword(testUser.reset_token, 'NuevaClave123!');
    console.log('✅ Clave actualizada mediante el token');

    await testUser.reload();
    console.log('Token post-reset:', testUser.reset_token); // debe ser null
